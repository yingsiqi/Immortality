...
> 涉及本机环境安装、团结编辑器启动或人工授权的任务，默认为“人工主导，AI 辅助”。

## TASK: P0-ENV-001 安装并验证 .NET SDK

> 任务属性：人工主导，AI 辅助。AI 负责检查环境、提供安装步骤、执行验证命令并回写阻塞项。
...
> 任务属性：人工主导，AI 辅助。AI 负责核对目录、依赖和回写结果，编辑器打开与版本选择由人工完成。
...
- 登录后调用 `/api/player/me`
- 修正当前 `GameManager.InitializeGame()` 登录成功后直接进入主界面的分支问题
...
- 确认首版创角 DTO 字段
- 若 `ApiClient` 尚无 `CreatePlayer()`，在本任务内一并补齐
- 客户端提交角色创建请求
...
- 若 `ApiClient` 尚无 `GetCultivationProgress()`，在本任务内一并补齐
- 客户端调用修炼进度接口
- 把数据映射到客户端模型
- 在修炼界面或调试面板上展示关键字段
- 验证无角色状态下的行为
